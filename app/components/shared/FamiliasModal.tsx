...
export function FamiliasModal({ visible, onClose, familias, onAdd, onUpdate, onRemove }: Props) {
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView style={styles.safe} edges={['top']}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>Familias</Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
          ...
          <View style={styles.list}>
            {familias.map((f) => {
              ...
              return (
                <View key={f.id} style={[styles.row, isEditing && styles.rowEditing]}>
                  <View style={[styles.colorDot, { backgroundColor: pal.fg }]} />
                  <Text style={styles.rowName}>{f.nombre}</Text>
                  <TouchableOpacity onPress={() => startEdit(f)} style={styles.rowBtn}>
                    <Ionicons name="pencil" size={16} color={colors.textSecondary} />
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => onRemove(f.id)} style={styles.rowBtn}>
          ...
          <View style={styles.form}>
            ...
            <TextInput
              style={styles.input}
              placeholder="Nombre (ej: Gym, Inglés...)"
            ...
            >
              <Text style={styles.saveBtnText}>
                {editingId ? 'Guardar cambios' : 'Agregar familia'}
              </Text>
            </TouchableOpacity>
            {editingId && (
...
const createStyles = (colors: AppColors) => StyleSheet.create({
  ...
  rowEditing: { borderWidth: 1.5, borderColor: colors.violet },
